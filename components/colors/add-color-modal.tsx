...
export function AddColorModal({ isOpen, onClose, dictionary }: AddColorModalProps) {
  ...
  const createColorMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: dictionary?.colors?.colorCreated || "Color created",
        description: dictionary?.colors?.colorCreatedDescription || "The color has been created successfully.",
      })
      handleClose()
    },
    onError: (error: Error) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{dictionary?.colors?.addColor || "Add Color"}</DialogTitle>
          <DialogDescription>
            {dictionary?.colors?.addColorDescription || "Add a new color to your store configuration."}
          </DialogDescription>
        </DialogHeader>
