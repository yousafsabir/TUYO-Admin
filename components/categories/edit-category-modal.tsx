...
const editCategorySchema = z.object({
  newName: z.string().min(1, "Category name is required").max(100, "Category name must be less than 100 characters"),
})
...
export function EditCategoryModal({ isOpen, onClose, categoryName, dictionary }: EditCategoryModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{dictionary?.categories?.editCategory || "Edit Category"}</DialogTitle>
          <DialogDescription>
            {dictionary?.categories?.editCategoryDescription || "Update the category name."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="newName">{dictionary?.categories?.newName || "New Category Name"}</Label>
            <Input
              id="newName"
              {...register("newName")}
              placeholder={dictionary?.categories?.namePlaceholder || "Enter category name"}
