  ...
  const setDefaultValues = async (values: Product) => {
    try {
      ...
      setFormData({
        ...
        startDate: values.startDate ? new Date(values.startDate).toISOString().slice(0, 16) : "",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border rounded-xl p-6">
        ...
        {errors.images && <p className="text-destructive text-sm mt-2">{errors.images}</p>}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="px-3 py-2 rounded-xl border">
          ...
          <Input
            ...
            className="border-0 px-0 focus-visible:ring-0 mt-1"
        ...
        <div className="px-3 py-2 rounded-xl border">
          ...
          <Select value={formData.brand} onValueChange={handleBrandChange}>
            <SelectTrigger className="border-0 px-0 focus:ring-0 mt-1">
        ...
        <div className="px-3 py-2 space-y-2 rounded-xl border">
          ...
          {errors.size && <p className="text-destructive text-sm">{errors.size}</p>}

          {/* Size Details Button */}
          <Button type="button" variant="outline" size="sm" onClick={() => setSizeSettingOpen(true)} className="w-full">
            Set Size Details
          </Button>
        </div>
        ...
        <div className="px-3 space-y-2 py-2 rounded-xl border">
              ...
              <Select value={formData.subcategory} onValueChange={(value) => updateField("subcategory", value)}>
                ...
                <SelectContent>
                  {categories[formData.category]?.subcategories &&
                    Array.isArray(categories[formData.category].subcategories) &&
                    categories[formData.category].subcategories.map((v: string) => (
          ...
          {errors.subcategory && <p className="text-destructive text-sm">{errors.subcategory}</p>}
        </div>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              {colorSelectionType === "predefined" && formData.color && (
                <span className="text-xs bg-muted px-2 py-1 rounded">Seleccione Predefinido</span>
              )}
            </div>
            ...
            <div className="space-y-2">
              {formData.color ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded-full border" style={{ backgroundColor: formData.colorCode }} />
                    <span className="text-sm">Ejemplo: {formData.color}</span>
                  </div>
                  <Button type="button" variant="outline" size="sm" onClick={() => handleColorSelection("custom")}>
                    Selecciona un color
                  </Button>
                </div>
              ) : (
                    ...
                    {Array.isArray(colors) &&
                      colors.map((color) => (
                        <SelectItem key={color[0]} value={color[0]}>
                          <div className="flex items-center gap-2">
                            <div className="w-4 h-4 rounded-full border" style={{ backgroundColor: color[1] }} />
                            {color[0]}
                          </div>
                        </SelectItem>
                      ))}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Input
                  ...
                  className="border-0 px-0 focus-visible:ring-0 flex-1 mr-3"
      ...
      <div className="px-3 py-2 rounded-xl border">
        ...
        <Textarea
          ...
          className="border-0 px-0 focus-visible:ring-0 min-h-[100px] mt-1"
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <Switch checked={formData.isFeatured} onCheckedChange={(checked) => updateField("isFeatured", checked)} />
          <Label>Featured Product</Label>
        </div>

        <div className="flex items-center gap-3">
          <Switch checked={formData.isPremium} onCheckedChange={(checked) => updateField("isPremium", checked)} />
          <Label>Premium Product</Label>
        </div>

        <div className="flex items-center gap-3">
          <Switch checked={formData.isAuction} onCheckedChange={(checked) => updateField("isAuction", checked)} />
      ...
      {formData.isAuction && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="px-3 py-2 rounded-xl border">
            <Label className="text-sm text-muted-foreground">Auction Start Date & Time</Label>
            <Input
              type="datetime-local"
              className="border-0 px-0 focus-visible:ring-0 mt-1"
          ...
          <div className="px-3 py-2 rounded-xl border">
            <Label className="text-sm text-muted-foreground">Auction End Date & Time</Label>
            <Input
              type="datetime-local"
              className="border-0 px-0 focus-visible:ring-0 mt-1"
