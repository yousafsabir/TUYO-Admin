...
interface DeleteSubcategoryDialogProps {
  isOpen: boolean
  onClose: () => void
  subcategory: { categoryId: number; categoryName: string; name: string } | null
  dictionary: any
}

export function DeleteSubcategoryDialog({ isOpen, onClose, subcategory, dictionary }: DeleteSubcategoryDialogProps) {
  ...
  const handleDelete = () => {
    if (subcategory) {
      setError(null)
      removeSubcategory({ categoryId: subcategory.categoryId, name: subcategory.name })
    }
  }
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={onClose}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{dictionary?.subcategories?.deleteSubcategory || "Delete Subcategory"}</AlertDialogTitle>
          <AlertDialogDescription>
            {dictionary?.subcategories?.deleteSubcategoryConfirmation ||
        ...
        <div className="mt-2 space-y-2 rounded-md bg-muted p-4">
          <div>
            <span className="font-medium">{dictionary?.subcategories?.name || "Name"}:</span> {subcategory.name}
          </div>
          <div>
            <span className="font-medium">{dictionary?.subcategories?.category || "Category"}:</span>{" "}
