...
export function SubscriptionsTable({ dictionary }: SubscriptionsTableProps) {
  const {
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        <AlertDescription>Error loading subscriptions. Please try again.</AlertDescription>
      </Alert>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {subscriptionsData?.data?.pagination && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          <div>
            Showing {subscriptions.length} of {subscriptionsData.data.pagination.total} subscriptions
          </div>
          <div>Page {subscriptionsData.data.pagination.page}</div>
        </div>
      )}
    </div>
  )
