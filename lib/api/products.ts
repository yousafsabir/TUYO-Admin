...
export async function getAllProducts(page = 1, limit = 25): Promise<ApiResponse<ProductsListResponse>> {
  try {
    const response = await fetchWithNgrok(`${API_BASE_URL}/products?page=${page}&limit=${limit}&sort=desc`, {
...
export async function updateProduct(productId: number, data: ProductUpdateData): Promise<ApiResponse<Product>> {
  try {
...
// Function to update a product
export async function updateProductFull(productId: number, data: FormData): Promise<ApiResponse<Product>> {
  try {
    const headers = createAuthHeaders(false)
    // Remove Content-Type completely for FormData
    delete headers["Content-Type"]
